...
const PAIRS = [
  ...
  ['默认大脑只会调用 nand_step 工具，不会自由问答，工具参数也可能不准确。想直接问问题，点上方的「门电路大脑」。',
    'The default brain only calls the nand_step tool; it cannot answer free-form questions, and tool arguments may be wrong. To ask questions, open “Gate brain” at the top.'],
  ...
  ['一个真正在你电脑上运行的 10 亿参数聊天模型。它不用任何 AI 推理库，而是被拆成几千亿个最简单的逻辑门，一位一位地算出每个字。需要一台电脑（手机一般跑不动）。',
    'A 1-billion-parameter chat model that really runs on your computer. It uses no AI inference library: it is broken down into hundreds of billions of the simplest logic gates and computes every word bit by bit. Needs a computer (phones usually cannot run it).'],
