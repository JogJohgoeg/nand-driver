...
const PAIRS = [
  ...
  ['它能直接问答，默认大脑不能。代价是慢，而且不接 Pi 代理：首次下载约 580 MB，在本机生成电路约 3–5 分钟；独立显卡每个 token 约 0.7 秒，笔记本集成显卡约 2.5 秒。提示加回答最多 128 个 token，装不下的上文会自动丢掉。',
    'It answers questions directly, which the default brain cannot. The cost is speed, and it is not wired to the Pi agent: first download is about 580 MB and building the circuit locally takes 3–5 minutes; about 0.7 s per token on a discrete GPU, about 2.5 s on a laptop integrated GPU. Prompt plus reply is at most 128 tokens; earlier turns that do not fit are dropped automatically.'],
